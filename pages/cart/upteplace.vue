<template>
	<view class="wrap">
		...
		<view class="addSite" @click="uptedata">
			<view class="add">
				<u-icon name="plus" color="#ffffff" class="icon" :size="30"></u-icon>更新地址
			</view>
		</view>
...
export default {
	...
	methods: {
		...
		async uptedata(){
			...
			await this.$u.api.upteplace(this.past.id,params)
			this.$u.route({
				type:'reLaunch',
	...
	onLoad(){
		const res=uni.getStorageSync('pastdata')
		if(res){
			this.past=res
		}
